...
function elite_preprocess_field(&$vars) {
    ...
    switch ( $vars['element']['#bundle'] ) {
      case 'article':
        if ( function_exists( 'elite_custom_check_taken_content' ) ) {
          $nid = $vars['element']['#object']->nid;
          $taken = elite_custom_check_taken_content( $vars['element']['#bundle'], $nid );
          //watchdog('fbpck', 'elite_custom_check_taken_content( '. $vars['element']['#bundle'] .' , '. $nid .' ) = '. $taken .'<br /><br /><pre>'. print_r($vars['element'],true) .'</pre>' );
          if ( $taken === 1 ) {
        ...
        if ( function_exists( 'elite_custom_check_taken_content' ) ) {
          if ( isset( $vars['element']['#object']->field_training_quiz['und'][0]['target_id'] ) ) {
            ...
            if ( $taken === 1 ) {
              ...
              if ( function_exists( 'elite_custom_check_required' ) ) {
                if ( elite_custom_check_required( $vars['element']['#object'] ) ) {
...
function elite_quiz_get_user_results($variables) {
  ...
  $required = false;
    ...
    if ( $tnode ) {
      if ( $tnode->nid > 0 ) {
        ...
        // check req
        $required = elite_custom_check_required( $tnode, $uid, $account );
  ...
  $counts = elite_custom_user_training_count( $uid, false, true );
 ...
 */
function elite_preprocess_user_profile(&$variables) {
 ...
 *
 */
function elite_quiz_progress($variables) {
  ...
  if ( function_exists( 'elite_custom_quiz_training' ) ) {
    $qnid = arg(1);
    //$output .= 'qnid '. $qnid .' ';
    $training = elite_custom_quiz_training( $qnid );
    if ( $training ) {
      if ( $training->nid ) {
