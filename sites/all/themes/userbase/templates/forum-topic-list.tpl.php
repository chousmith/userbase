...
$forum_topic = taxonomy_term_load( $topic_id );
//print_r($forum_topic);
print $forum_topic->name;
}
?>
</div>
<table id="forum-topic-<?php print $topic_id; ?>">
  <thead>
    <tr><?php
    // first, replace the default first blank th spacer for deleted td.icon
    $header = str_replace( '<th></th>', '', $header );
    // then change "Last reply" to "Recent Reply"
    $header = str_replace( t( 'Last reply' ), t( 'Recent Reply' ), $header );
    // and then print
    print $header;
    ?>
    </tr>
